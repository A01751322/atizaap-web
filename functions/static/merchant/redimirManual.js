/* eslint-env browser */
// functions/static/merchant/redimirManual.js
(function() {
  "use strict";
 ...
 */
  ...
  /**
   * Formatea una tarjeta a "xxxx-xxxx-xxxx-xxxx".
   * @param {string} [value=""] - Valor a formatear.
   * @return {string} Valor formateado.
   */
  const formatCard16 = (value = "") => {
  ...
  async function submitRedemption(idNegocio, idOferta, cardNumber) {
    ...
    try {
      ...
      const cardInput = $("#card-number-input");
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const inputCard = $("#card-number-input");
    if (inputCard) {
      /* Formateo en vivo */
      inputCard.addEventListener("input", (e) => {
        ...
        const beforeLen = currentVal.length;

        const formatted = formatCard16(currentVal);
        input.value = formatted;

        const afterLen = formatted.length;
        const isDash = formatted.charAt(cursorPos - 1) === "-";
        if (afterLen > beforeLen && isDash) {
      ...
      inputCard.addEventListener("paste", (e) => {
        ...
        const paste = clip.getData("text");
        inputCard.value = formatCard16(paste);
      });
    }
    ...
    if (submitBtn) {
      submitBtn.addEventListener("click", () => {
        ...
        if (!idNegocio) {
          paintResponse(
              "error",
              "Error de autenticación. Refresca la página.",
          );
          return;
        }
        if (!idOferta) {
    ...
    if (cancelBtn) {
      cancelBtn.addEventListener("click", () => {
        ...
        const selectOffer = $("#oferta-select");
        if (selectOffer) selectOffer.selectedIndex = 0;
        const resp = $("#response");
